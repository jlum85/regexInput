...
const FieldDetail = (props) => {
  ...
  const handleNew = () => {
    ...
    const newField = {
      ...
      pattern: "",
      description: "",
      patternDetail: [{ id: 1, pattern: "", message: "", expected: true }],
    };
  ...
  const onInsertOrUpdate = (event) => {
    ...
    try {
      ...
      alert("Expression régulière non valide : " + pattern);
      return;
    }
    ...
    const newField = {
      ...
      pattern: regex,
      description,
      patternDetail: regexDetail.filter((item) => item.pattern),
    };
    if (createMode) {
  ...
  const updateRow = (row, col, value) => {
    ...
    for (let i = 0; i < result.length; i++) {
      if (result[i].id === row) {
        if (col === 1) {
          try {
            result[i].pattern = StringToRegex(value);
          } catch (e) {
            alert("Expression régulière non valide : " + value);
            return;
          }
        } else if (col === 2) {
    ...
    <>
      {!field ? (
        ...
        <div className="detail">
          <form className="formDetail" onSubmit={onInsertOrUpdate}>
            {createMode ? (
              <>
                <div className="headerCreate">
                  <h2>Création nouveau Input</h2>
